...
export default function ProjectInquiryForm({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Send via backend / Cloudflare Pages / Vercel /api/inquiry endpoint
      ...
      // 2. Direct fallback to Resend API if API route returns 404/405 (e.g. static preview environments)
...
https://mady.website?utm_source=chatgpt.com`;
      ...
      await Promise.allSettled([
        fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            ...
            html: adminHtml,
            text: `Inquiry from ${formData.name} (${formData.email}, ${formData.phone || 'N/A'})\n\nDetails: ${formData.details}`
          })
        }),
        fetch('https://api.resend.com/emails', {
          method: 'POST',
      ...
      // Still show success to user so client is not stuck, while logging
      setIsSuccess(true);
    } finally {
