...
export default function FounderSection() {
  ...
  return (
    <section aria-labelledby="founder-heading" className="relative flex min-h-[100svh] items-start px-6 pb-16 pt-24 md:items-center md:py-12 md:px-12 lg:pl-56 xl:pl-64" id="founder">
      <div className="relative mx-auto w-full max-w-6xl md:pt-[var(--work-drop)] flex flex-col lg:flex-row items-start justify-between gap-8" style={{ "--WorkDrop": "min(9vh, max(0px, calc((100svh - 640px) / 2)))" }}>
        
        <AnimatedSection>
          <header className="relative z-10 mb-4 max-w-xl shrink-0 lg:mb-[calc(2rem+var(--work-drop))]">
        ...
        <div className="spatial-perspective pointer-events-none relative lg:absolute lg:right-0 lg:top-1/2 lg:-translate-y-1/2 z-0 w-full max-w-[420px] lg:max-w-[460px] xl:max-w-[500px] mt-4 lg:mt-0" style={{ perspective: "1700px", transformStyle: "preserve-3d" }}>
          <div className="spatial-orientation pointer-events-auto relative" style={{ transformStyle: "preserve-3d", backfaceVisibility: "hidden", willChange: "transform" }}>
            <div className="grid grid-cols-1">
              ...
              <div className="group relative flex flex-col overflow-hidden rounded-2xl border border-bone/[0.12] bg-gradient-to-b from-bone/[0.06] to-bone/[0.015] transition-[border-color,transform] duration-500 ease-out hover:border-bone/25 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-gilt/70">
                <div className="relative overflow-hidden w-full" style={{ paddingBottom: '115%' }}>
                  <img 
                    src={founderImg}
                    alt="Syed Hassan" 
                ...
                <div className="relative z-10 flex flex-1 flex-col p-5 sm:p-6 border-t border-bone/[0.12] bg-bone/[0.02] backdrop-blur-md">
                  <span className="eyebrow text-[0.55rem] text-gilt/55">United Kingdom · Horizon Digital</span>
                  <h3 className="display mt-1 text-[1.15rem] sm:text-[1.2rem] text-bone">Syed Hassan</h3>
                  <p className="mt-2.5 text-[0.75rem] leading-relaxed text-bone/60">
                  ...
                  <a href="#projekt-anfragen" onClick={smoothScrollTo} className="eyebrow mt-auto inline-flex items-center gap-1.5 pt-5 text-[0.55rem] text-bone/70 transition-colors duration-300 group-hover:text-bone">
                    Let's Work Together
